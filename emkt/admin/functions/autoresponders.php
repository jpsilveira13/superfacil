...
class Autoresponders extends SendStudio_Functions
{
	...
	*/
	function Process()
	{
        ...
        $tempID = (isset($_GET['id'])) ? strtolower($_GET['id']) : null;
        $tempID = (isset($_POST['autoresponders'])) ? strtolower($_POST['autoresponders']) : $tempID;
		if (!is_null($tempID) && !is_array($tempID)) {
            $tempID = array($tempID);
		}
		...
		$user = GetUser();
		$tempCheckActions = array('activate', 'deactivate', 'pause', 'resume', 'copy', 'change', 'delete', 'step2', 'sendpreview', 'view', 'edit');
       ...
       if(!$user->Admin() && in_array($action, $tempCheckActions)){
                if(!is_null($tempID)){
                    foreach ($tempID as $tempEachID) {
                        ...
                        if($user->userid == $tempAPI->ownerid){
                            $access = true;
                        } else {
                            $access = $user->HasAccess('Autoresponders',$action,$tempEachID);   
                        }
                     }  
                } else {
