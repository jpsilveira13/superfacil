<?php
	...
	class ISC_PRODUCT
	{
		...
		public $prodData = array();//array used for addon customprodpackage
		...
		public function _SetProductData($productid=0)
		{
			...
			if ($productid == 0) {
				...
				$productSQL = sprintf("p.prodmyobincome='%s'", $product);
			}
			else {
			...
			if($pull_count != 0 ){
				 $this->prodData = array(
						'prodvariationid'		=>	$this->_prodvariationid,
						'prodoptionsrequired'	=> $this->_prodoptionsrequired
						);
			}
			...
			$GLOBALS['CurrentProductLink'] = ProdLink($row['prodmyobincome']);
		}
		...
		*/
		...
		public function ShowPage()
		{
			if ($this->_prodid > 0) {
				...
				$GLOBALS['ISC_CLASS_TEMPLATE']->SetCanonicalLink(ProdLink($this->_product['prodmyobincome']));
				...
				$GLOBALS['AdditionalMetaTags'] = ISC_OPENGRAPH::getMetaTags($this->_opengraph_type, $this->_opengraph_title, $this->_opengraph_description, $this->_opengraph_image, ProdLink($this->_product['prodmyobincome']));
		...
		*/
		...
		static public function buildSearchResultsHTML()
		{
			...
			foreach ($results["results"] as $product) {
				...
				$GLOBALS["ProductLink"] = ProdLink($product['prodmyobincome']);
				...
				if (GetConfig('ShowProductPrice') && !$row['prodhideprice']) {
					$GLOBALS['ProductPrice'] = formatProductCatalogPrice($product);
					$GLOBALS['ProductParcela'] = MostraParcela($product['prodprice']);
				}

				// Price OFF
				$preco = $product['prodprice'];
				$precoDesconto = $product['prodretailprice'];
				...
				$GLOBALS["ProductThumb"] = ImageThumb($product, ProdLink($product['prodmyobincome']), "", "TrackLink");

				if (isId($product["prodvariationid"]) || trim($product["prodconfigfields"]) !== "" || $product["prodeventdaterequired"] == 1) {
					$GLOBALS["ProductURL"] = ProdLink($row['prodmyobincome']);
				...
				if (CanAddToCart($product) && GetConfig("ShowAddToCartLink")) {
					...
					$GLOBALS['HideIndisponivel'] = 'none';
				} else {
					$GLOBALS["HideActionAdd"] = "none";
					$GLOBALS['HideIndisponivel'] = 'block';
				}
		...
		static public function buildSearchResultsAJAX()
		{
			...
			foreach ($products as $product) {
				...
				$GLOBALS["ProductURL"] = ProdLink($product['prodmyobincome']);
				$GLOBALS['ProductPrice'] = '';
				if (GetConfig('ShowProductPrice') && !$product['prodhideprice']) {
					$GLOBALS['ProductPrice'] = formatProductCatalogPrice($product);
				}
