	...
	class ISC_ACCOUNT
	{
		...
		*/
		private function SaveEditedShippingAddress()
		{
			if (isset($_POST['shipid'])) {
				...
				$shippingData = $this->shippingEntity->get($_POST['shipid'], $customerId);
				if (!$shippingData) {
					ob_end_clean();
					header(sprintf("location: " . $GLOBALS['ShopPath'] . "/account.php"));
					die();
				}
		...
		*/
		...
		private function SaveNewShippingAddress()
		{
			...
			if (isset($ShippingAddress['shipfirstname']) && isset($ShippingAddress['shipaddress1'])) {
				// ISC-4034 me being paranoid, making absolutely sure customer id cannot be overridden -ge
				$ShippingAddress['shipcustomerid'] = $GLOBALS['ISC_CLASS_CUSTOMER']->GetCustomerId();
		...
		*/
		private function DeleteShippingAddress()
		{
			if (isset($_GET['address_id'])) {
				...
				$address = $this->shippingEntity->get($_GET['address_id'], $GLOBALS['ISC_CLASS_CUSTOMER']->GetCustomerId());
				if (!$address) {
					ob_end_clean();
					header(sprintf("location:%s/account.php", $GLOBALS['ShopPath']));
					die();
				...
				if ($this->shippingEntity->delete($_GET['address_id'])) {
					if (isset($_SESSION['LOGIN_REDIR'])) {
						// Take them to the page they wanted
						$page = $_SESSION['LOGIN_REDIR'];
						unset($_SESSION['LOGIN_REDIR']);
		...
		*/
		public function GetShippingAddress($addressId, $customerId=null)
		{
			...
			$shippingAddresses[$addressId] = $this->shippingEntity->get($addressId, $customerId);
		...
		private function SendMessage()
		{
			...
			if (isset($_POST['message_order_id']) && isset($_POST['message_subject']) && isset($_POST['message_content'])) {
				...
				############
					...
					$ccname = $GLOBALS['ISC_CLASS_CUSTOMER']->GetCustomerName();
					#$_POST['message_content']
					
					$GLOBALS['CustomerName'] = $ccname;
					$emailTemplate = FetchEmailTemplateParser();
					$emailTemplate->SetTemplate("ordermessagecustomer_notification");
					...
					$obj_email->From($ccemail);
					$obj_email->Set("Subject", $store_name." New Order Message Order ID: ".(int)$_POST['message_order_id']);
		...
		*/
		private function OrderStatus()
		{
			...
			while ($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
				...
				$GLOBALS['OrderId'] = $row['orderid'];
				$GLOBALS['OrderTotal'] = CurrencyConvertFormatPrice($row['total_inc_tax'], $row['ordcurrencyexchangerate'], true);
		...
		*/
		private function ViewOrderDetails()
		{
			...
			$query = "
				...
				LEFT JOIN [|PREFIX|]order_products op ON (op.orderorderid = o.orderid)
				LEFT JOIN [|PREFIX|]products p ON p.productid = op.ordprodid
			...
			foreach ($OrderProducts as $row) {
				...
				if ($prod_name == "" && $row['ordprodtype'] == 'giftcertificate') {
					...
					$GLOBALS['Link'] = ProdLink($row['prodmyobincome']);
					...
					if ($GLOBALS['ISC_CLASS_DB']->CountResult($result) > 0) {
						...
						while ($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
							...
							if (GetConfig('ShowProductPrice') && !$row['prodhideprice']) {
								...
								$GLOBALS['ProductParcela'] = MostraParcela($row['prodprice']);
							}

							// Price OFF
							$preco = $row['prodprice'];
							$precoDesconto = $row['prodretailprice'];

							if($precoDesconto > $preco ){
								$GLOBALS['HavePriceOFF'] = "block";
								$GLOBALS['PriceOFF'] = number_format((($preco / $precoDesconto) * 100) - 100, 0)."%" ;
							} else {
								$GLOBALS['HavePriceOFF'] = "none";
							}
