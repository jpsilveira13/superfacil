		...
		try {
			...
			if($count != 0){ 
			 $query = $GLOBALS['ISC_CLASS_DB']->Query("
					select gbprodsleft from [|PREFIX|]groupbuy_prods where productid = '".$product_id."'
					and gbstatus = 1
					");
					$row = $GLOBALS['ISC_CLASS_DB']->Fetch($query);
					$query2 = $GLOBALS['ISC_CLASS_DB']->Query("
					select count(*) from [|PREFIX|]groupbuy_prods where productid = '".$product_id."' and gbstatus = 1
					");
					$count = $GLOBALS['ISC_CLASS_DB']->FetchOne($query2);
					if($count != 0){
						$items = $this->getQuote()->getItems();
						foreach($items as $item) {
							if($product_id == $item->getProductId()){
								if(($item->getQuantity() + $qty) > $row['gbprodsleft']){
									$gb_qty  = false;
									exit();
								} 
							}						
			...
			if($e->getCode() == ISC_QUOTE_EXCEPTION::ERROR_NO_STOCK && $showMinQuantityAdjustment) {
				...
				)), MSG_ERROR, prodLink($product['prodmyobincome']));
			}
			else {
				flashMessage($e->getMessage(), MSG_ERROR, prodLink($product['prodmyobincome']));
			}
		}
