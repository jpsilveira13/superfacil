	...
	class ISC_CUSTOMER
	{
		...
		*/
		public function GetCustomerName()
		{
			...
			if ($customer_id > 0) {
				$query = sprintf("select concat(custconlastname, ', ', custconfirstname) as fullname from [|PREFIX|]customers where customerid='%d'", $GLOBALS['ISC_CLASS_DB']->Quote($customer_id));
				$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
