<?php
	...
	CLASS ISC_CATEGORYTOP_PANEL extends PANEL
	{
		public function SetPanelSettings()
		{
			...
			$GLOBALS['SNIPPETS']['CategoryTopList'] = $output;
		}
		...
		*/
		protected function _getSubCategory($categories, $parentCatId)
		{
			...
			if ($output!='') {
				...
				$output = '<ul class="dropdown-menu">'.$output.'</ul>';
			}
			return $output;
		}
		...
		protected function _generateClassicOutput ()
		{
			...
			foreach($categories[0] as $rootCat) {
				...
				$GLOBALS['CategoryDropDown'] = "dropdown";
				// @todo ul here is hacky but front end templates are limited, fix this when possible
				$output .= '<ul class="nav navbar-nav">' . $GLOBALS['ISC_CLASS_TEMPLATE']->GetSnippet("CategoryTopList") . '</ul>';
			}
