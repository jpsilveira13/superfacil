...
CLASS ISC_PRODUCTDETAILS_PANEL extends PANEL
{
	...
	*/
	...
	private function SetPricingDetails()
	{
		...
		// Funcoes de Parcelamento
		$GLOBALS['PagamentoParcelado'] = MostraParcela($product['prodcalculatedprice']);
		$numero_parcela = GetConfig('ParcelaComJuros');
		$GLOBALS["TabelaParcela1"].='<table border="0" cellspacing="0" width="100%" cellpadding="0">';
		for($parcela=1;$parcela<=$numero_parcela;$parcela++){
			$GLOBALS["TabelaParcela1"] .= Criaparcelas($product['prodcalculatedprice'],$parcela);
		}
	...
	private function SetPurchasingOptions()
	{
		...
		if (GetConfig('TagCartQuantityBoxes') == 'dropdown') {
			...
			if($count != 0){
				$checkid = $GLOBALS['ISC_CLASS_DB']->Query("
				select count(*) from [|PREFIX|]groupbuy_prods where productid = '".$GLOBALS['ProductId']."' and gbstatus = 1
				");
				$count = $GLOBALS['ISC_CLASS_DB']->FetchOne($checkid);
				if($count == 0){
					//original code here
					...
					select * from [|PREFIX|]groupbuy_prods where gbstatus = 1 
						and productid = '".$this->productClass->GetProductId()."'
					");
					$row = $GLOBALS['ISC_CLASS_DB']->Fetch($query);
					$GLOBALS['gbindicator'] = "
					<link rel='stylesheet' type='text/css' href='%%GLOBAL_ShopPath%%/javascript/jquery/plugins/jquerycountdown/jquery.countdown.css'> 
					<style type='text/css'>
					.countdown {width:120px; height: 18px;}
					</style>
					<script type='text/javascript' src='%%GLOBAL_ShopPath%%/javascript/jquery/plugins/jquerycountdown/jquery.countdown.js'></script>
					...
					$('div.countdown').each(function(){
						...
						end_date = new Date(parseInt(arr[0]),parseInt(arr[1])-1,parseInt(arr[2]),parseInt(arr[3]),parseInt(arr[4]),parseInt(arr[5]));			
						$(this).countdown({until: end_date, timezone: %%GLOBAL_gbtimezone%%}); 		
					...
					<style type='text/css'>
					.countdown {width:220px; height: 45px;}
					</style>
					<div class='Label'><h3>GROUP BUY ITEM:</h3></div>
					<div class='value'>".$row['gbprodsleft']." items left for group buying</br>
					<em><font size=1>Note: You cannot add to cart more than the number of left items set for groupbuy of this product.</font></em></div></br>
					<div class='Label'>Remaining Time:</div>
					<div class = 'value'>
					<div class='countdown'>".isc_date('Y-m-d-h-i-s',$row['gbendtime'])."</div></div>
					";
	...
	private function SetProductVariations()
	{
		...
		foreach($variationOptions as $optionName) {
            ...
            if(strtolower($optionName) == 'cor' || strtolower($optionName) == 'colour' || strtolower($optionName) == 'cores' || strtolower($optionName) == 'colours') $color = 1;
//            if(strtolower($optionName) == 'size') $size = 1;

			// Build the list of options
			$GLOBALS['OptionChecked'] = '';
			$GLOBALS['SNIPPETS']['OptionListColor'] = '';
            $GLOBALS['IndentColor'] = '';
			if (isset($variationValues[$optionName])) {
				foreach($variationValues[$optionName] as $optionid => $value) {
					...
					if($onlyOneVariation && !$useSelect) {
                        if($color == 1){
                            $GLOBALS['OptionValue'] = str_ireplace('HEX:','#',$GLOBALS['OptionValue']);
                            $GLOBALS['IndentColor'] = ' IndentColor';
                            $GLOBALS['SNIPPETS']['OptionListColor'] .= $GLOBALS['ISC_CLASS_TEMPLATE']->GetSnippet("ProductVariationListColorItem");
                            $GLOBALS['SNIPPETS']['OptionList'] .= $GLOBALS['ISC_CLASS_TEMPLATE']->GetSnippet("ProductVariationListSingleItem");
                        }
                        else{
                            $GLOBALS['SNIPPETS']['OptionList'] .= $GLOBALS['ISC_CLASS_TEMPLATE']->GetSnippet("ProductVariationListSingleItem");
			...
			if($onlyOneVariation == true && !$useSelect) {
                        if($color == 1){
                            $GLOBALS['IndentColor'] = ' IndentColor';
                            $output = $GLOBALS['ISC_CLASS_TEMPLATE']->GetSnippet("ProductVariationListColor");
				...
				$GLOBALS['VariationChooseText'] = GetLang('ChooseA')." ".isc_html_escape($optionName);
                        if($color == 1){
                            $GLOBALS['IndentColor'] = ' IndentColor';
                            $output = $GLOBALS['ISC_CLASS_TEMPLATE']->GetSnippet("ProductVariationListColor");
            				$output .= $GLOBALS['ISC_CLASS_TEMPLATE']->GetSnippet("ProductVariationListMultiple2");
                        }
                        else{
	...
	public function LoadEventDate()
	{
		...
		if (empty($edlimited)) {
			...
			if ($fields['prodeventdatelimitedtype'] == 1) {
				...
				$comp_date = isc_date('d/m/Y', $from_stamp);
				$comp_date_end = isc_date('d/m/Y', $to_stamp);
				...
				$comp_date = isc_date('d/m/Y', $to_stamp);
	...
	private function GetDayOptions()
	{
		...
		for($i = 1; $i <= 31; $i++) {
			...
			$output .= sprintf("<option %s value='%d'>%s</option>",$select, $i, $i);
		}
	...
	private function GetMonthOptions()
	{
		...
		for($i = 1; $i <= 12; $i++) {
			...
			$output .= sprintf("<option %s value='%d'>%s</option>",$select, $i, $mes_extenso[$i]);
		}
	...
	public function SetMiscAttributes()
	{
		...
		if (GetConfig('ShowAddThisLink')) {
			$GLOBALS['UrlDoProduto'] = $this->urlAtual();
