...
class ISC_CARTCONTENT_PANEL extends PANEL
{
	...
	public function generateCartContent()
	{
		...
		foreach($items as $item) {
			...
			$result = $GLOBALS['ISC_CLASS_DB']->Query("SELECT prodmyobincome FROM [|PREFIX|]products WHERE productid = '$id'");
			...
			$GLOBALS['ProductLink'] = ProdLink($row['prodmyobincome']);
			...
			if(getConfig('ShowThumbsInCart')) {
				$GLOBALS['ProductImage'] = imageThumb($item->getThumbnail(), '');
			}
			...
			if(!empty($eventDate)) {
				$GLOBALS['EventDate'] = '
					<div style="font-style: italic; font-size:10px; color:gray">(' .
						$item->getEventName() . ': ' . isc_date('d/m/Y', $eventDate) .
					')</div>';
			}
			...
			// If we're using a cart quantity drop down, load that
				...
				if($count != 0){
					$gb_addon_enabled = true;
					 //check if product id is in groupbuy prods table -> use $GLOBALS['ItemId']
				$checkid = $GLOBALS['ISC_CLASS_DB']->Query("
				select count(*) from [|PREFIX|]groupbuy_prods where productid = '".$GLOBALS['ItemId']."' and gbstatus = 1
				");
				...
				if($count == 0){
					...
					select gbprodsleft from [|PREFIX|]groupbuy_prods where productid = '".$GLOBALS['ItemId']."'
					and gbstatus = 1
					");
					$row = $GLOBALS['ISC_CLASS_DB']->Fetch($query);
					
					
						for($x=1;$x<=$row['gbprodsleft'];$x++){
	
							$selected = $x == $quantity ? 'selected="selected"' : '';
							$this->setgbqty($x, $selected);
							
							$output.=$GLOBALS['ISC_CLASS_TEMPLATE']->GetSnippet("groupbuycartselect");
						}
						$GLOBALS['SNIPPETS']['groupbuycartselect'] = $output;
						#$GLOBALS['QtyGrpbuyOptionSelected'] = '<option selected="'.$quantity.'">'.$quantity.'</option>';
						$GLOBALS['gbqtypanel'] = '%%Panel.groupbuycartselect%%';
						$GLOBALS['CartItemQty'] = " ";
					}
				}
		...
		if($gb_addon_enabled == true){
			 $script2 = "
			$('.gbquantityInput').live('change', function() {
				Cart.UpdateQuantity($(this).val());
			});
			";
			$GLOBALS['ISC_CLASS_TEMPLATE']->clientScript->registerScript($script2,'ready');		
		}
		//end groupbuy addon code
