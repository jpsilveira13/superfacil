<?php
/*
	Cron Rastreamento dos Correios
...
*/
	...
	// Funcao para remover acentuação
	function limpar($val)
	{
	...
	// Funcao para fazer o rastreamento
	function rastrear($codigo)
	{
		...
		if($pos_inicial > 0) {
			...
			// Faz a limpeza de acentos
			$resultado['status'] = limpar($status);
	...
	{
		require_once(ISC_BASE_PATH . "email.php");
	...
	// Verifica se o addon esta ativo ou nao
	if($ativo=='sim')
	{
		$urlsite = GetConfig('ShopPath');
		$nomeloja = GetConfig('StoreName');
		$a = "select * from [|PREFIX|]orders AS a,[|PREFIX|]customers AS b,[|PREFIX|]shipments AS c WHERE b.customerid=a.ordcustid AND a.ordstatus IN('2','3') AND a.orderid=c.shiporderid AND length(c.shiptrackno) > 10";
		$b = $GLOBALS['ISC_CLASS_DB']->Query($a);
		$total = $GLOBALS['ISC_CLASS_DB']->CountResult($a);

		// Verifica se encontrou entregas
		if($total>0)
		{
			// Faz Loop pegando os itens encontrados
			while($dados = $GLOBALS['ISC_CLASS_DB']->Fetch($b))
			{
				$objeto = str_replace(' ','',$dados['shiptrackno']);
				...
				if($row['status']!='Erro')
				{
					...
					$objeto = str_replace(' ','',$dados['shiptrackno']);
					$emails = str_replace(' ','',$dados['custconemail']);
					$nomes = $dados['custconfirstname']." ".$dados['custconlastname'];
