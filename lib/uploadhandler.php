...
class UploadHandlerFile {
	...
	*/
	...
	public function moveAs($destination, $overwrite = true)
	{
		...
		$checkcontent = $GLOBALS['ISC_CLASS_DB']->Query("Select count(imgsrc) from [|PREFIX|]watermark_settings ");
			if($GLOBALS['ISC_CLASS_DB']->FetchOne($checkcontent)>0){
			$watermarkquery = sprintf('select imgsrc, position from [|PREFIX|]watermark_settings order by watermarkid desc limit 1');
			$watermarkresult = $GLOBALS['ISC_CLASS_DB']->Query($watermarkquery);
				$getrow = $GLOBALS['ISC_CLASS_DB']->Fetch($watermarkresult);
				$WaterMarkImg = $getrow['imgsrc'];
				$Position = $getrow['position'];
			$SourceFile = $destination;				
			$f_type = $this->getExtension();		
			...
			$imgovlay = imagecreatefrompng($WaterMarkImg);
					$marge_right = 0;
					$marge_bottom = 0;
					$sx = imagesx($imgovlay);
					$sy = imagesy($imgovlay);
					switch($Position){
						case 'Bottom Left':
							imagecopy($image_p, $imgovlay, 0, imagesy($image_p) - $sy - $marge_bottom, 0, 0, imagesx($imgovlay), imagesy($imgovlay));
						break;
						case 'Bottom Right':
							imagecopy($image_p, $imgovlay, imagesx($image_p) - $sx - $marge_right, imagesy($image_p) - $sy - $marge_bottom, 0, 0, imagesx($imgovlay), imagesy($imgovlay));
						break;
						case 'Top Left':
							imagecopy($image_p, $imgovlay, 0, 0, 0, 0, imagesx($imgovlay), imagesy($imgovlay));
						break;
						case 'Top Right':
							imagecopy($image_p, $imgovlay, imagesx($image_p) - $sx - $marge_right, 0, 0, 0, imagesx($imgovlay), imagesy($imgovlay));
						break;
						case 'Center':
							imagecopy($image_p, $imgovlay, (imagesx($image_p) - $sx - $marge_right)/2, (imagesy($image_p) - $sy - $marge_bottom)/2, 0, 0, imagesx($imgovlay), imagesy($imgovlay));
						break;
						case 'Middle Left':
							imagecopy($image_p, $imgovlay, 0, (imagesy($image_p) - $sy - $marge_bottom)/2, 0, 0, imagesx($imgovlay), imagesy($imgovlay));
						break;
						case 'Middle Right':
							imagecopy($image_p, $imgovlay, (imagesx($image_p) - $sx - $marge_right), (imagesy($image_p) - $sy - $marge_bottom)/2, 0, 0, imagesx($imgovlay), imagesy($imgovlay));
						break;
						default:
