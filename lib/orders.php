...
function EmailInvoiceToCustomer($orderId, $newStatusId=null)
{
	...
	// Puxa Data do Pedido
	$GLOBALS['DataPedido'] = CDate($order_row['orddate']);

	// Puxa informacoes de logomarca da loja
	$GLOBALS['LogoEmail'] = '<img src="'.GetConfig('ShopPath')."/".GetConfig('ImageDirectory')."/".GetConfig('StoreLogo').'" />';

	// Puxa informacoes da forma de pagamento
	$GLOBALS['FormaPagamento'] = $order_row['orderpaymentmethod'];
	...
	if ($order_row['shipping_address_count'] > 1) {
		...
		$GLOBALS['ShippingAddress'] = $emailTemplate->GetSnippet("AddressLabel2");
	}
	...
	$GLOBALS['BillingAddress'] = $emailTemplate->GetSnippet("AddressLabel2");
	...
	$GLOBALS['TotalCost'] = FormatPrice($order_row['total_inc_tax'], false, true, false, $defaultCurrency, false);
	...
	foreach($order_row['products'] as $product_row) {
		...
		$GLOBALS['ProductPrice'] = FormatPrice($product_row[$itemPriceColumn], false, true, false, $defaultCurrency, false);
		$GLOBALS['ProductTotal'] = FormatPrice($product_row[$itemTotalColumn], false, true, false, $defaultCurrency, false);
	...
	foreach($totalRows as $row) {
		...
		$emailTemplate->assign('value', formatPrice($row['value'], false, true, false, $defaultCurrency, false));
 ...
 * @param int The optional ID of the order status. Will default to the already stored status ID of the order
 */
function EmailAbandonedCartToCustomer($orderId, $coupons, $yousave, $subjectline, $newStatusId=null)
{
	// Load the details for this order
	$order_row = GetOrder($orderId, true);
	if($order_row === false) {
	...
	if($coupons != 'none'){
		$GLOBALS['CouponMessage'] = sprintf("We would like to offer you a special discount towards your next purchase! <br />
Simply enter the coupon code <b>%s</b> at the checkout and you will save %s on your current pending order.<br /><br />", $coupons, $yousave);
		#$GLOBALS['YouSave']	= $yousave;
	}

	$emailTemplate->SetTemplate("abandonedcart_email");
	$message = $emailTemplate->ParseTemplate(true);

	// Create a new email API object to send the email
	$store_name = GetConfig('StoreName');
	#sprintf(GetLang('YourOrderFrom'), $store_name)
	
	$obj_email = GetEmailClass();
	$obj_email->From(GetConfig('OrderEmail'), $store_name);
	$obj_email->Set("Subject", $subjectline);
	$obj_email->AddBody("html", $message);
	$obj_email->AddRecipient($email, "", "h");
	$email_result = $obj_email->sendAsynchronous();
	...
	foreach($forwardEmails as $address) {
		...
		$emailClass->Set('CharSet', GetConfig('CharacterSet'));
		$emailClass->From(GetConfig('OrderEmail'), $store_name);
		$emailClass->Set("Subject", "Fwd: ".$subjectline);
		$emailClass->AddBody("html", $message);
		$emailClass->AddRecipient($address, "", "h");
		$emailClass->sendAsynchronous();
	}
...
function DecreaseInventoryFromOrder($orderId)
{
	...
	while ($product = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
		...
		$gb_addon_enabled = false;
		if($count != 0){
			$gb_addon_enabled = true;
			///////set 1 as value for isgroupbuy in [|PREFIX|]prder_products for a groupbuy product name with ref to [|PREFIX|]groupbuy_prods
			$getgbdetails = $GLOBALS['ISC_CLASS_DB']->Query("
				select productid,gbprodname,minbuyers,maxbuyers,percentdisc,discountedprice,currentdiscount,
				currentpurchases from [|PREFIX|]groupbuy_prods where currentorderid = '".$product['orderorderid']."'
			");
			$row=$GLOBALS['ISC_CLASS_DB']->Fetch($getgbdetails);
				$prodname = $row['gbprodname'];
				$GLOBALS['ISC_CLASS_DB']->Query("
				update [|PREFIX|]order_products set isgroupbuy = 1
				where ordprodname = '".$prodname."' and orderorderid = '".$product['orderorderid']."'
				");	
			///////end for setting up isgroupbuy = 1
		}
		//end groupbuy addon code
	...
	//groupbuy addon code
	if($gb_addon_enabled == true){
		////////update gbprods here
	$getfromordprod = $GLOBALS['ISC_CLASS_DB']->Query("
	select ordprodqty,orderorderid from [|PREFIX|]order_products
	where orderorderid = '".$orderId."' and isgroupbuy = 1
	");
	$rowordprod = $GLOBALS['ISC_CLASS_DB']->Fetch($getfromordprod);
	$updbuyers = $GLOBALS['ISC_CLASS_DB']->Query("
			update [|PREFIX|]groupbuy_prods set 
			currentpurchases = currentpurchases +".$rowordprod['ordprodqty'].",
			gbprodsleft = gbprodsleft - ".$rowordprod['ordprodqty']."
			where currentorderid = '".$rowordprod['orderorderid']."' 
		");
	$getgbdetails = $GLOBALS['ISC_CLASS_DB']->Query("
			select productid,gbprodname,minbuyers,maxbuyers,percentdisc,discountedprice,currentorderid,currentdiscount,
			currentpurchases from [|PREFIX|]groupbuy_prods where currentorderid = '".$rowordprod['orderorderid']."'
		");
		$row=$GLOBALS['ISC_CLASS_DB']->Fetch($getgbdetails);
			$prodname = $row['gbprodname'];
			$pid = $row['productid'];
			$currentpurchases = $row['currentpurchases'];
			$currentdiscount = $row['currentdiscount'];
			$minbuyers = unserialize($row['minbuyers']);
			$maxbuyers = unserialize($row['maxbuyers']);
			$percentdisc = unserialize($row['percentdisc']);
			$discountedprice = unserialize($row['discountedprice']);
			$mintemp = array();
			$maxtemp = array();
				for($x=0;$x<count($minbuyers['minbuyers']),$x<count($maxbuyers['maxbuyers']);$x++){
					$mintemp[$x] = $minbuyers['minbuyers'][$x];
					$maxtemp[$x] = $maxbuyers['maxbuyers'][$x];
					var_dump($discountedprice);
					if(($currentpurchases>=$mintemp[$x]) && ($currentpurchases<=$maxtemp[$x])){
						//updates discount at given index when that index value is currently true as to what the condition states
							$query = $GLOBALS['ISC_CLASS_DB']->Query("
							update [|PREFIX|]groupbuy_prods,[|PREFIX|]products 
							set [|PREFIX|]groupbuy_prods.currentdiscount = '".$discountedprice[$x]."',
							[|PREFIX|]products.prodprice = '".$discountedprice[$x]."',
							[|PREFIX|]products.prodcalculatedprice = '".$discountedprice[$x]."'
							where [|PREFIX|]groupbuy_prods.productid = [|PREFIX|]products.productid 
							and [|PREFIX|]groupbuy_prods.productid = '".$pid."'
							");
							#break; 
							//update [|PREFIX|]order_products
							$GLOBALS['ISC_CLASS_DB']->Query("
							update [|PREFIX|]order_products set 
							base_price= '".$discountedprice[$x]."' ,
							price_ex_tax= '".$discountedprice[$x]."' ,
							price_inc_tax= '".$discountedprice[$x]."',
							base_total= '".$discountedprice[$x]."' * ordprodqty,
							total_ex_tax= '".$discountedprice[$x]."' * ordprodqty,
							total_inc_tax= '".$discountedprice[$x]."' * ordprodqty
							where isgroupbuy = 1
							and ordprodname = '".$row['gbprodname']."'
							and orderorderid = '".$row['currentorderid']."'
					");
							//update [|PREFIX|]orders - try to update [|PREFIX|]orders at every min point, instead of after min point
		...
		//get qty and orderorderid per product in [|PREFIX|]order_products
		...
		while($row = $GLOBALS['ISC_CLASS_DB']->Fetch($query4)){
			...
			if((isc_date('Y-m-d-h-i-s') >= isc_date('Y-m-d-h-i-s',$row['gbendtime'])) || ($row['currentpurchases'] >= end($maxbuyers['maxbuyers']))){
				...
				");
				//update [|PREFIX|]order_products and [|PREFIX|]orders
...
$str = "select loyaltybased as amount, loyaltypoints as points from [|PREFIX|]loyaltyrewards";
$sql = $GLOBALS['ISC_CLASS_DB']->Query($str);
$row = $GLOBALS['ISC_CLASS_DB']->Fetch($sql);
return $row; 
}

function isLoyalty(){
	
	$str = "select variableid from [|PREFIX|]module_vars where modulename='addon_loyaltyrewards' and variableval=1";
	$sql = $GLOBALS['ISC_CLASS_DB']->Query($str);
	if($GLOBALS["ISC_CLASS_DB"]->CountResult($sql) == 1) {
		return true;			
	}else{
		return false;	
	}
...
function AddLoyaltyPoints($customerid='', $amount='', $action='add'){
		$sql = "select custstorecredit from [|PREFIX|]customers where customerid='".$customerid."'";
		$found = $GLOBALS['ISC_CLASS_DB']->Query($sql);
		$row = $GLOBALS['ISC_CLASS_DB']->Fetch($found);
		
		$loyaltyrewards = getLoyaltyRewards();
		$loyaltypoints = number_format($loyaltyrewards['points'],2,'.','');
		$loyaltyamount = number_format($loyaltyrewards['amount'],2,'.','');			
		$customercredit = ($amount / $loyaltyamount) * $loyaltypoints;
		if($action == 'add'){
			$customercredit = number_format($row['custstorecredit'] + $customercredit,4,'.','');
		}else{
			$customercredit = number_format($row['custstorecredit'] - $customercredit,4,'.','');
		}
		
		$sql = "UPDATE [|PREFIX|]customers set custstorecredit='".$customercredit."' where customerid='".$customerid."'";
...
function UpdateOrderStatus($orderIds, $status, $email=true, $preventModuleUpdateCallback=false)
{
	...
	foreach($orderIds as $orderId) {
		...
		/*$sql = "select custstorecredit from [|PREFIX|]customers where customerid='".$order['ordcustid']."'";
		$found = $GLOBALS['ISC_CLASS_DB']->Query($sql);
		$row = $GLOBALS['ISC_CLASS_DB']->Fetch($found);
		
		if($order['ordstatus'] == 2 || $order['ordstatus'] == 10){
			if($status != 2 || $status != 10){
				$loyaltyrewards = getLoyaltyRewards();
				$loyaltypoints = number_format($loyaltyrewards['points'],2,'.','');
				$loyaltyamount = number_format($loyaltyrewards['amount'],2,'.','');			
				$customercredit = ($order['total_inc_tax'] / $loyaltyamount) * $loyaltypoints;
				$customercredit = number_format($row['custstorecredit'] - $customercredit,4,'.','');
				$sql = "UPDATE [|PREFIX|]customers set custstorecredit='".$customercredit."' where customerid='".$order['ordcustid']."'";
				$GLOBALS['ISC_CLASS_DB']->Query($sql);
			} 
		...
		if(isLoyalty()){
			if($order['ordstatus'] == 2 || $order['ordstatus'] == 10){
				AddLoyaltyPoints($order['ordcustid'], $order['total_inc_tax'], 'sub');
			}
		...
		if (OrderIsComplete($status)) {
			...
			if(isLoyalty()){				
				AddLoyaltyPoints($order['ordcustid'], $order['total_inc_tax'], 'add');
			}
		}
...
function EmailOnStatusChange($orderId, $status)
{
	...
	$GLOBALS['OrderNumber'] = $order['orderid'];
	$GLOBALS['OrderStatus'] = $statusName;
	...
	$GLOBALS['OrderTotal'] = FormatPrice($order['total_inc_tax'], false, true, false, $defaultCurrency, false);
	...
	while($shipment = $GLOBALS['ISC_CLASS_DB']->Fetch($shipments)) {
		...
		if ($module) {
			...
			if ($link) {
				$link = 'Código de Rastreio: <a href="' . isc_html_escape($link) . '" target="_blank">' . $shipment['shiptrackno'] . '</a><br />';
			} else {
		...
		if($shipment['shipmethod']) {
			$link .= 'Entregue por <strong>' . $shipment['shipmethod'] .'</strong>';
		}

		if ($link) {
			$GLOBALS['TrackingLinkList'] .= $link;
		}
	}
	...
	if (empty($GLOBALS['TrackingLinkList'])) {
		...
		$GLOBALS['TrackingLinkList'] = $GLOBALS['TrackingLinkList'];
	}
...
function SubscribeCustomerToLists($pendingOrderToken)
{
	...
	foreach($orders['orders'] as $order) {
		...
		if(!ISC_EMAILINTEGRATION::doOrderAddRulesExist() || (isset($extraInfo['join_order_list']) && $extraInfo['join_order_list'])) {
	...
	if($order['shipping_address_count'] > 1) {
		...
		if (!empty ($freeShippingCoupons)) {
			foreach($freeShippingCoupons as $key=>$val) {
				$totalRows[$key] = $val;
				$totalRows['shipping']['value'] += abs($val['value']);
			}
