				...
				switch($GLOBALS["PageType"]) {
					...
					case "search_page": {
						...
						if(isset($GLOBALS["Banners"]["top"])) {
                  		...
                  		if(isset($GLOBALS["Banners"]["right"])) {
                     		// Replace it out
                     		$tplData = str_replace("%%Banner.RightBanner%%", $GLOBALS["Banners"]["right"]["content"], $tplData);
                  		}else {
                     		// Replace it with nothing
                     		$tplData = str_replace("%%Banner.RightBanner%%", "", $tplData);
                  		}

                  		// Is there a left template?
                  		if(isset($GLOBALS["Banners"]["left"])) {
                     		// Replace it out
                     		$tplData = str_replace("%%Banner.LeftBanner%%", $GLOBALS["Banners"]["left"]["content"], $tplData);
                  		}else {
                     		// Replace it with nothing
                     		$tplData = str_replace("%%Banner.LeftBanner%%", "", $tplData);
                  		}
