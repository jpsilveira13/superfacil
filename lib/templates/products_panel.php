...
class PRODUCTS_PANEL extends PANEL
{
	...
	public function setProductGlobals($row)
	{
		...
		$GLOBALS['ProductThumb'] = ImageThumb($row, ProdLink($row['prodmyobincome']));
	...
	public function setPackagesGlobals($row){
		$GLOBALS['imagefile']		= $row['imagefiletiny'];
		$GLOBALS['packagename'] 	= $row['package_name'];
		$GLOBALS['main_prod_id'] 	= $row['main_prod_id'];
		$GLOBALS['num_prods']		= $row['num_prods'];
		$GLOBALS['main_prod']		= $row['prodname'];
		$GLOBALS['sub_prods_ids']	= $row['sub_prods_ids'];
		$GLOBALS['ProductLink'] 	= ProdLink($row['prodname']);
	}
	public function setDealProdsGlobals($dealprodsrow){
		$GLOBALS['prodname'] = $dealprodsrow['dealprodname'];
		$GLOBALS['getid'] = $dealprodsrow['productid'];
		$GLOBALS['imgfile'] = $dealprodsrow['imagefile'];
		$GLOBALS['dprice'] = number_format($dealprodsrow['dealprice'],2);
		$GLOBALS['dateto'] = isc_date('Y-m-d-h-i-s',$dealprodsrow['dealdateto']);
	}
}