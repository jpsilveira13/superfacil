...
function generatePrintableInvoice($orderId)
{
	...
	if($row['shipping_address_count'] > 1) {
		...
		$template->assign('hideInvoiceShippingDetails', '');
	}
...
function generatePrintablePackingSlip($title, $details, $products)
{
	...
	// Modificacao da Etiqueta
	$idform = $details['shipcustid'];
	$queryform = "SELECT formfieldfieldvalue FROM sole_customers
	LEFT JOIN sole_shipping_addresses ON sole_customers.customerid = sole_shipping_addresses.shipcustomerid
	INNER JOIN sole_formfieldsessions ON sole_shipping_addresses.shipformsessionid = sole_formfieldsessions.formfieldsessioniformsessionid
	WHERE customerid = '$idform'";
	$resultform = mysql_query($queryform) or die(mysql_error());
	$rowform = mysql_fetch_array($resultform);
	$numero = explode('"', $rowform['formfieldfieldvalue']);
	$addressDetails = array(
		'shipfirstname'	=> $details['shipbillfirstname'],
		'shiplastname'	=> $details['shipbilllastname'],
		'shipcompany'	=> $details['shipbillcompany'],
		'shipaddress1'	=> $details['shipbillstreet1'].", ".$numero[1],
	...
	$addressDetails = array(
		...
		'shipaddress1'	=> $details['shipshipstreet1'].", ".$numero[1],
...
function getInvoiceShippingAddressBlock($address)
{
	...
	$template->assign('ShippingPhone',isc_html_escape($address['phone']));
