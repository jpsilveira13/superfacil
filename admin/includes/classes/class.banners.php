	...
	class ISC_ADMIN_BANNERS extends ISC_ADMIN_BASE
	{
		...
		public function HandleToDo($Do)
		{
			switch (isc_strtolower($Do)) {
				case "editbanner2": {
					if ($this->auth->HasPermission(AUTH_Manage_Banners)) {
			...
			while ($banner = $GLOBALS["ISC_CLASS_DB"]->Fetch($bannerResult)) {
            	...
            	switch ($banner['location'])
            	{
              		...
              		case "bottom":{
               		...
               		case "right":{
	               		$GLOBALS['Location'] .= sprintf(" (%s)", GetLang('BannerRightOfPage'));
	               		break;
               		}
              		case "left":{
	               		$GLOBALS['Location'] .= sprintf(" (%s)", GetLang('BannerLeftOfPage'));
	               		break;
               		}
               	}
			...
			if (isset($_GET['bannerId']) && is_numeric($_GET['bannerId'])) {
				...
				if ($banner = $this->GetBannerById($banner_id)) {
					...
					if ($banner['page'] == "category_page") {
						$categoryId = $banner['catorbrandid'];
						$this->template->assign('bannerCategory', $categoryId);
					}

					$GLOBALS['CategoryOptions'] = ISC_ADMIN_CATEGORY::getJSTreeCategoryData($categoryId, 0, false);
					...
					else if ($banner['location'] == "right") {
                  		$GLOBALS['IsLocationRight'] = "selected=\"selected\"";
               		}
					else if ($banner['location'] == "left") {
                  		$GLOBALS['IsLocationLeft'] = "selected=\"selected\"";
               		}
					else {
