<?php
class ISC_ADMIN_ABANDONED_CART extends ISC_ADMIN_BASE
{
		...
		public function HandleToDo($Do)
		{
			...
			$GLOBALS['BreadcrumEntries'] = array(
				...
				'Pedidos Abandonados' => 'index.php?ToDo=viewAbandoned'
			);
			...
			if(!gzte11(ISC_LARGEPRINT)) {
				...
				$GLOBALS['ISC_CLASS_ADMIN_ENGINE']->DoHomePage(GetLang('Unauthorized'), MSG_ERROR);
					...
					{
						case 'updatemultiabandonedorder':
		...
		protected function SendAbandonedCartNotification()
		{
			if (array_key_exists('orders', $_REQUEST)) {
				#$GLOBALS['StatusID'] = $_REQUEST['statusId'];
				#$GLOBALS['JavaScriptOrderIds'] = $_REQUEST['orders'];
				#$GLOBALS['CouponCodes'] = $_REQUEST['couponcodes'];
				$pieces = explode(",", $_REQUEST['orders']);
				$ids = '';
				if($_REQUEST['couponcodes'] != 'none'){
					$coupon = $this->getCouponType($_REQUEST['couponcodes']);
					
					switch($coupon['coupontype']){
						case '0':
							$yousave = '<b>'.GetConfig('CurrencyToken').number_format($coupon['couponamount'], 2, '.', '').'</b> each item ';
						break;
						case '1':
							$yousave = '<b>'.number_format($coupon['couponamount'], 2, '.', '').'</b> percent each item ';
						break;
						case '2':
							$yousave = '<b>'.GetConfig('CurrencyToken').number_format($coupon['couponamount'], 2, '.', '').'<b> off the order total '; 
						break;
						case '3':
							$yousave = '<b>'.GetConfig('CurrencyToken').number_format($coupon['couponamount'], 2, '.', '').'</b> off the shipping total '; 
						break;
						case '4':
				...
				foreach($pieces as $orderid){
					#$ids .= $v.'<br>';
					#$ids .= $_REQUEST['subjectline'].' --- '.$orderid.' xxx '.$yousave.' aaa '.$this->GetCustomerEmailByOrderId($orderid).'<br>';
					switch($_REQUEST['subjectline']){
						case '1':
							$subjectline = sprintf(GetLang('AbandonedCartEmail1'), GetConfig('StoreName'));
						break;
						case '2':
							$subjectline = sprintf(GetLang('AbandonedCartEmail2'), $this->GetCustomerNameOnly($orderid));
						break;
						case '3':
							$subjectline = sprintf(GetLang('AbandonedCartEmail3'), $this->GetCustomerNameOnly($orderid), GetConfig('StoreName') );
						break;
						case '4':
							$subjectline = sprintf(GetLang('AbandonedCartEmail4'), $this->GetCustomerNameOnly($orderid), GetConfig('StoreName') );
						break;
						case '5':
							$subjectline = GetLang('AbandonedCartEmail5');
						break;	
					}
					EmailAbandonedCartToCustomer($orderid, $_REQUEST['couponcodes'], $yousave, $subjectline);
				}
				$GLOBALS['Ord_ids'] = $ids;

				$this->template->display('orders.abandoned.popup.tpl');
			}
		}
		...
		public function getCouponType($coupon){
			...
			$query = sprintf("select coupontype, couponamount from [|PREFIX|]coupons where couponcode='".$coupon."'");
		...
		*/
		...
		protected function ManageOrdersGrid(&$numOrders, &$numDeletedOrders = 0)
		{
			...
			$GLOBALS['pageURL'] = "index.php?ToDo=viewAbandoned&" . http_build_query($searchURL);
			...
			BuildAdminSortingLinks($sortLinks, "index.php?ToDo=viewAbandoned&amp;".http_build_query($sortURL)."&amp;page=".$page, $sortField, $sortOrder);
			...
			return $this->template->render('abandoned.manage.grid.tpl');
		}
		...
		protected function ManageOrders($MsgDesc = "", $MsgStatus = "")
		{
			...
			$GLOBALS['AbandonedSubjectLine'] = '<option selected="1" value="none">Select Subject Line</option>';
			$GLOBALS['AbandonedSubjectLine'] .= '<option value="1">'.GetLang('AbandonedCartEmail1').'</option>';
			$GLOBALS['AbandonedSubjectLine'] .= '<option value="2">'.GetLang('AbandonedCartEmail2').'</option>';
			$GLOBALS['AbandonedSubjectLine'] .= '<option value="3">'.GetLang('AbandonedCartEmail3').'</option>';
			$GLOBALS['AbandonedSubjectLine'] .= '<option value="4">'.GetLang('AbandonedCartEmail4').'</option>';
			$GLOBALS['AbandonedSubjectLine'] .= '<option value="5">'.GetLang('AbandonedCartEmail5').'</option>';
			
			$GLOBALS['Coupons'] = '<option selected="1" value="none">Select Coupon</option>';
			
			$result = $GLOBALS['ISC_CLASS_DB']->Query("SELECT couponid, couponname, couponcode FROM [|PREFIX|]coupons ORDER BY couponid DESC");
			while ($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
				$GLOBALS['Coupons'] .= '<option value="' . $row['couponcode'] . '">' . $row['couponname'] . '</option>';
			}
			...
			if (isset($searchGet['searchId']) && $searchGet['searchId'] == 0) {
				...
				// expected when the clicking 'view: all orders' which is '&ToDo=viewAbandoned&searchId=0'
				unset($searchGet['searchId']);
			}
			...
			$GLOBALS['CustomSearchOptions'] = $GLOBALS['ISC_CLASS_ADMIN_CUSTOMSEARCH']->GetSearchesAsOptions($_REQUEST['searchId'], $num_custom_searches, "AllOrders", "viewAbandoned", "customOrderSearch");
			...
			if($numOrders == 0) {
				...
				/*if(count($searchGet) > 1) {
					...
					$GLOBALS['Message'] = MessageBox('No Abandoned Carts/Orders have been placed from your website. When an Abandoned Carts/Orders comes through it will appear on this page. ', MSG_SUCCESS);
			...
			$this->template->display('abandoned.manage.tpl');
		}
		...
		*/
		...
		protected function UpdateOrderMessageStatus()
		{
			if (isset($_GET['orderId']) && isset($_GET['messageId']) && isset($_GET['status'])) {
				...
				if($this->auth->GetVendorId() && $order['ordvendorid'] != $this->auth->GetVendorId()) {
					FlashMessage(GetLang('Unauthorized'), MSG_ERROR, 'index.php?ToDo=viewAbandoned');
				}
		...
		protected function FlagOrderMessage()
		{
			if(isset($_GET['flagState']) && isset($_GET['orderId']) && isset($_GET['messageId'])) {
				...
				if ($order['deleted']) {
					FlashMessage(GetLang('OrderDeletedGeneralNotice'), MSG_ERROR, 'index.php?ToDo=viewAbandoned');
				}
		...
		public function GetCustomerNameOnly($OrderId)
		{
			$query = sprintf("select ordcustid, (select concat(custconfirstname, ' ', custconlastname) from [|PREFIX|]customers where customerid=ordcustid) as custname, (select custconemail from [|PREFIX|]customers where customerid=ordcustid) as custemail  from [|PREFIX|]orders where orderid='%d'", $GLOBALS['ISC_CLASS_DB']->Quote($OrderId));
			$result = $GLOBALS['ISC_CLASS_DB']->Query($query);

			if ($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
				return sprintf("%s", $row['custname'], $row['custemail']);
			} else {
				return sprintf(GetLang('CustomerForOrderX'), $OrderId);
			}
		}
		...
		*/
		...
		{
			#SetSession('ordersearch', 1);

			#if ($_GET['searchId'] > 0) {
				$this->_customSearch = $GLOBALS['ISC_CLASS_ADMIN_CUSTOMSEARCH']->LoadSearch(1);
		...
		*/
		...
		protected function RefundOrder()
		{
			...
			if($message != '') {
				FlashMessage($message, $messageStatus, 'index.php?ToDo=viewAbandoned');
			}
