...
class ISC_ADMIN_STATISTICS_OVERVIEW extends ISC_ADMIN_STATISTICS
{
	...
	*/
	public function _Top20ProductsGrid($FromStamp, $ToStamp, &$NumResults)
	{
		...
		$vendorRestriction = '';
		if($this->auth->GetVendorId()) {
			$vendorRestriction = " AND prodvendorid='" . $this->auth->GetVendorId() . "'";
		}
		...
		if($NumResults > 0) {
			...
			while($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
				$prodlink = $row['prodmyobincome'];
				if (!is_null($row['productid'])) {
					$prodlink = "<a href='" . ProdLink($row['prodmyobincome']) . "' target='_blank'>" . isc_html_escape($row['ordprodname']) . "</a>";
				}
	...
	public function OverviewOrderLocationChart()
	{
		...
		if(isset($_GET['from']) && isset($_GET['to'])) {
			...
			if($num_orders > 0) {
                ...
                $mapScript = "http://maps.googleapis.com/maps/api/js?v=3.4&sensor=false&key=". GetConfig('GoogleMapsKey');
                ?>
                    <html>
                        <head>
                            <style>
                            ...
                            <script type="text/javascript">
                                ...
                                function DrawOverlay(info, results) {
                                
                                    var infoWindowContent = "<div style='font-weight: bold; font-size: 11px;'>"+info.name+" (<a target='_parent' href='index.php?ToDo=viewCustomers&amp;searchQuery="+info.id+"' style='color:blue'><?php echo GetLang('ViewOrderHistory'); ?></a>)</div><br />"+info.suburb+", "+info.state+"<br />"+info.country+" "+info.zip;
				...
				?>
				<script type="text/javascript">
					alert('<?php echo GetLang('StatsNoOrdersForDate'); ?>');
				</script>
				<?php
			}
		}
