	...
	class ISC_ADMIN_PRODUCT extends ISC_ADMIN_BASE
	{
		...
		public function ManageProductsGrid(&$numProducts)
		{
			...
			if($numProducts > 0) {
				...
				while($row = $GLOBALS["ISC_CLASS_DB"]->Fetch($product_result)) {
					...
					$GLOBALS['Name'] = sprintf("<a title='%s' class='Action' href='%s' target='_blank'>%s</a>", GetLang('ProductView'), ProdLink($row['prodmyobincome']), isc_html_escape($row['prodname']));
		...
		*/
		public function _GetProductList($Start, $SortField, $SortOrder, &$NumProducts, $fields='', $limit = ISC_PRODUCTS_PER_PAGE)
		{
			...
			if($fields == '') {
				$fields = "prodmyobincome, p.productid, p.prodname, p.prodvariationid, p.prodprice, p.prodallowpurchases, p.prodpreorder, prodinvtrack, p.prodcode, p.proddesc, IF(prodinvtrack = 0, 0, prodcurrentinv) AS currentinv, prodvisible, prodlowinv, prodcalculatedprice, p.prodfeatured, p.prodvendorfeatured, p.tax_class_id, t.*";
			}
		...
		*/
		...
		public function EditProductStep2()
		{
			...
			if ($this->_CommitProduct($prodId, $arrData, $arrVariations, $arrCustomFields, $discount, $err, $arrProductFields)) {
				$successMessage = sprintf(GetLang('ProductUpdatedSuccessfully'), isc_html_escape($arrData['prodname']), ProdLink($arrData['prodmyobincome']));
				if ($this->auth->HasPermission(AUTH_Manage_Products)) {
		...
		public function EditProductStep1($MsgDesc = "", $MsgStatus = "", $PreservePost=false)
		{
			...
			if (ProductExists($prodId)) {
				...
				$optimizer = getClass('ISC_ADMIN_OPTIMIZER');
				$GLOBALS['OptimizerConfigForm'] = $optimizer->showPerItemConfigForm('product', $prodId, prodLink($arrData['prodmyobincome']));
		...
		*/
		...
		public function CopyProductStep1($MsgDesc = "", $MsgStatus = "", $PreservePost=false, $OriginalProductID=0)
		{
			...
			if (ProductExists($prodId)) {
				...
				$optimizer = getClass('ISC_ADMIN_OPTIMIZER');
				$GLOBALS['OptimizerConfigForm'] = $optimizer->showPerItemConfigForm('product', $arrData['productid'],prodLink($arrData['prodmyobincome']));
