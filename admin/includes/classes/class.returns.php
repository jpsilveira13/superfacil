...
class ISC_ADMIN_RETURNS extends ISC_ADMIN_BASE
{
	...
	public function HandleToDo($Do)
	{
		...
		switch (isc_strtolower($Do))
		{
			case "createreturnview":
				if ($this->auth->HasPermission(AUTH_Manage_Returns)) {
	...
	private function IssueReturnCredit()
	{
		...
		$creditLog = array(
			...
			"credituserid" => $this->auth->GetUserId(),
	...
	public function UpdateReturnStatus(&$return, $status, $crediting = false)
	{
		...
		if($status == 5 && $return['retstatus'] != 5) {
			...
			if($remainingItems == 0) {
				$updatedOrder = array(
					'ordstatus' => ORDER_STATUS_REFUNDED
				);
				$GLOBALS['ISC_CLASS_DB']->UpdateQuery("orders", $updatedOrder, "orderid='".$GLOBALS['ISC_CLASS_DB']->Quote($return['retorderid'])."'");
			}

			// has inventory been returned for this request already?
			$inventoryReturned = $return['inventory_returned'];
			if (!$return['inventory_returned']) {
				// Update the product inventory for this returned item
				$query = "SELECT prodinvtrack FROM [|PREFIX|]products WHERE productid = " . (int)$return['retprodid'];
				$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
				$product = $GLOBALS['ISC_CLASS_DB']->Fetch($result);
				if ($product) {
					AdjustProductInventory($return['retprodid'], $return['retprodvariationid'], $product['prodinvtrack'], '+'.$return['retprodqty']);
					$inventoryReturned = 1;
				}
			}
		...
		if(GetConfig('NotifyOnReturnStatusChange') == 1) {
			$this->EmailReturnStatusChange($return, $crediting);
		}
	...
	private function EmailReturnStatusChange($return, $crediting=false)
	{
		...
		if($return['retstatus'] == 5 && $crediting) {
	...
	public function ManageReturns($MsgDesc = "", $MsgStatus = "")
	{
		...
		if (!$this->auth->HasPermission(AUTH_Manage_Returns) || $numReturns == 0) {
	...
	private function _GetReturnsList($Start, $SortField, $SortOrder, &$NumReturns)
	{
		...
		if($this->auth->GetVendorId()) {
			$queryWhere .= " AND retvendorid='".(int)$this->auth->GetVendorId()."'";
		}
