		...
		*/
		...
		protected function ManageOrdersGrid(&$numOrders, &$numDeletedOrders = 0)
		{
			...
			while ($row = $GLOBALS['ISC_CLASS_DB']->Fetch($orderResult)) {
				...
				// Verifica se existe um comprovante de deposito
				$dir = "../lib/sendComprovante";

				// Abre um diretorio conhecido, e faz a leitura de seu conteudo
				if (is_dir($dir)) {
				    if ($dh = opendir($dir)) {
				        while (($file = readdir($dh)) !== false) {
				            if($file == "comprovante_".$row['orderid'].".jpg"){
				            	$comprovante = '<a style="margin-left: 20px;" href="../lib/sendComprovante/comprovante_'.$row['orderid'].'.jpg"" target="_blank"><i class="fa fa-search"></i> Ver Comprovante</a>';
				            } else {
				            	$comprovante = "";
				            }
				        }
				...
				if(trim($row['custname']) != '') {
					$GLOBALS['CustomerLink'] = "<a href='index.php?ToDo=viewCustomers&amp;idFrom=".$GLOBALS['CustomerId']."&idTo=".$GLOBALS['CustomerId']."'>".$GLOBALS['Customer']."</a>".$comprovante;
				}
				else {
