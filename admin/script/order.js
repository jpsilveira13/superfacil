...
function HandleAbandonedAction(){
	var sel = $('#OrderActionSelect').val();
	var subjectsel = $('#AbandonedEmailActionSelect').val();
	if(subjectsel == 'none'){
		alert("Please select Subject line");
		return false;	
	}
	...
	if (orders.length > 0) {
		tb_show('', 'index.php?ToDo=updateMultiAbandonedOrder&subjectline=' + subjectsel + '&couponcodes=' + sel + '&orders=' + orders.join(',') + '&TB_iframe=true&height=120&width=400&modal=false&random='+new Date().getTime(), '');
	} else {
		alert('Please choose at least one order to send email notification.');
	}	
