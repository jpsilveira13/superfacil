...
var CurrentVersion = '{{ TemplateVersion|raw }}';

function ShowTab(T){
	i = 0;
	if('{{ HideMessageBox|raw }}' == 'none'){
...
{
	window.open('{{ ShopPathNormal|raw }}/?designModeToken={{ DesignModeToken|raw }}');
}
...
function DownloadNewVersion(){
	if(confirm('Important Note: By downloading this new template you will completely override your current template files which will *not* be recoverable. If you have made any modifcations to your current template then you should backup your current template before continuing.\n\nTo download this template, click \'OK\'. To keep the current version, click the \'Cancel\' button.')){
		if($.browser.msie){
			tb_show('', "index.php?ToDo=templatedownload&template={{ CurrentTemplateName|raw }}&color={{ CurrentTemplateColor|raw }}&height=80&width=280&PreviewImage={{ CurrentTemplateImage|raw }}");
		}else{
			tb_show('', "index.php?ToDo=templatedownload&template={{ CurrentTemplateName|raw }}&color={{ CurrentTemplateColor|raw }}&height=58&width=240&PreviewImage={{ CurrentTemplateImage|raw }}");
		}
...
<script type="text/javascript" src="script/layout.headerimage.js?{{ JSCacheToken }}"></script>
	...
	<table class="OuterPanel">
		...
		<ul id="tabnav">
			...
			<input id="currentTab" name="currentTab" value="{{ ShowTab|raw }}" type="hidden">
			</form>
	...
	<div id="div0">
		...
		<p class="MessageBox MessageBoxInfo" style="{{ HideSafeModeMessage|raw }}; margin-top: 10px;">{% lang 'TemplateDownloadingSafeModeEnabled' %}</p>
		...
		<table class="Panel">
			...
			<tr>
				<td align="left" width="200" style="padding:5px 5px 5px 10px;">
					<a href='{{ ShopPath|raw }}/templates/{{ CurrentTemplateName|raw }}/Previews/{{ CurrentTemplateImage|raw }}' class="thickbox"><img src="thumb.php?tpl={{ CurrentTemplateName|raw }}&color={{ CurrentTemplateImage|raw }}" border="0" id="CurrentTemplateImage"></a>
				</td>
				<td align="left" valign="top"  style="padding:5px 5px 5px 10px;">
					<div class="TemplateHeading" id="CurrentTemplateHeading">{{ CurrentTemplateNameProper|raw }} ({{ CurrentTemplateColor|raw }}) - Version {{ TemplateVersion|raw }}</div>
					<div id="TemplateFilesLocated">{% lang 'TemplateFilesLocated' %}{{ CurrentTemplateName|raw }}</div><br />
				</td>
			</tr>
	 </table><br />
	...
	<table class="Panel" style="margin:0px;">
		...
		<div id="div2" style="display:none">
			...
			<p>
				<input type="button" class="btn" onclick="launchDesignMode();" value="{% lang 'LaunchDesignMode' %}" />
			</p>
		</div>
				...
				<table class="Panel" style="margin:0px;">
					...
					<tr>
						...
						<td class="PanelBottom">
							<img src="{{ Favicon|raw }}" width="16" height="16" />&nbsp;&nbsp;<input type="file" name="FaviconFile" id="FaviconFile" class="Field" value="" /> <input type="submit" class="btn" value="{% lang 'UploadFavicon' %}" />
						</td>
					</tr>
				</table>
			</form>
