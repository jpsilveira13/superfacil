...
{% block common %}
	...
	{{ forms.startForm() }}
		...
		{{ forms.startRow({
			...
			<input type="button" class="{{ module.id }}_verifyApiKey btn" value="Verificar Detalhes de API" />
			&nbsp;
			...
			<span class="apiConfiguredContainer" {% if not module.object.isConfigured %}style="display:none;"{% endif %}>
				&nbsp;
				<input type="button" class="button {{ module.id }}_refreshLists btn bnt-atualize" value="{{ lang.EmailIntegrationRefreshLists }}" />
			</span>

			<span class="apiNotConfiguredContainer" {% if module.object.isConfigured %}style="display:none;"{% endif %}>
				&nbsp;
				<input type="button" class="button {{ module.id }}_refreshLists btn btn-atualize" value="{{ lang.EmailIntegrationRefreshLists }}" disabled="disabled" title="{{ lang.VerifyApiDetailsFirst }}" />
			</span>
		{{ forms.endRow }}
