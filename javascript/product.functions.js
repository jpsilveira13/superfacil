...
function updateSelectedVariation(parent, variation, id) {
	...
	// Otherwise, showing a specific variation
		...
		$('.ValorParcelado', parent).each(function(){
			var $$ = $(this);
			if ($$.is('input')) {
				$$.val(variation.price.replace(/[^0-9\.,]/g, ''));
			} else {
				$$.html($.ajax({ url: '../lib/parcelamento.php?s='+variation.price, success: function(data) { $$.html(data); } }));
			}
		});


		$('.Parcelamento', parent).each(function(){
			var $$ = $(this);
			if ($$.is('input')) {
				$$.val(variation.price.replace(/[^0-9\.,]/g, ''));
			} else {
				$$.html($.ajax({ url: '../lib/tabela_parcelamento.php?s='+variation.price, success: function(data) { $$.html(data); } }));
			}
		});
